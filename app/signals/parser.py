...
def _parse_english_signal(message: NormalizedMessage) -> SignalCandidate | None:
    ...
    if not symbol_raw or not side:
        return None
    order_type = "market" if _market_entry(flat) else "limit"
    ...
    missing = []
    if not entries and order_type != "market":
        missing.append("entry")
    if not take_profits:
        missing.append("take_profit")
    if stop_loss is None:
        missing.append("stop_loss")
    parsed = bool(entries or order_type == "market")
    confidence = 0.95 if not missing else 0.92 if parsed else 0.68
    return SignalCandidate(
        source_log_id=message.log_id,
        category="new_signal",
        raw_text=message.raw_text,
        ...
        status="parsed" if parsed else "needs_review",
    )
...
def _market_entry(text: str) -> bool:
    return bool(re.search(r"\b(BUY|SELL)\s+NOW\b", text, flags=re.I))
