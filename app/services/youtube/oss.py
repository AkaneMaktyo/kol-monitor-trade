...
class AliyunOssUploader:
    ...
    def sign_video_audio(self, video_id: str) -> str:
        bucket = self._bucket()
        object_key = f"{self._prefix}/{video_id}.aliyun.wav"
        if not bucket.object_exists(object_key):
            return ""
        return bucket.sign_url("GET", object_key, self._expire_seconds)
    ...
    def _bucket(self):
        ...
        return oss2.Bucket(oss2.Auth(self._ak_id, self._ak_secret), self._endpoint, self._bucket_name)
