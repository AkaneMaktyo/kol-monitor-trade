(function () {
    ...
    async function load() {
        ...
        if (!response.ok) throw new Error("交易决策台加载失败");
    ...
    function renderReadiness() {
        ...
        if (!data) return;
        refs.mode.textContent = data.live_mode ? "真实提交就绪" : data.execution_mode === "dry_run" ? "当前为 dry_run" : "交易未就绪";
        refs.mode.className = `trading-mode-pill ${data.live_mode ? "pill-good" : data.trading_enabled ? "pill-warn" : "pill-bad"}`;
        refs.summary.innerHTML = [
            summaryBox("交易开关", data.trading_enabled ? "已开启" : "已关闭", data.product_type),
            summaryBox("执行模式", data.execution_mode || "--", `${data.max_signal_age_seconds}s 时效窗`),
            summaryBox("凭证状态", data.credential_status === "ready" ? "已就绪" : "未就绪", data.credential_message || ""),
            summaryBox("风险预算", `${num(data.risk_budget.quote_risk_usdt)} USDT`, `${num(data.risk_budget.risk_percent)}% / 权益 ${num(data.risk_budget.account_equity_usdt)}`),
            summaryBox("最近下单", statusLabel(data.last_order && data.last_order.status), lastOrderText(data.last_order)),
        ].join("");
    }
    ...
    function renderSignal(row) {
        return item(`${esc(row.symbol)} ${sideLabel(row.side)}`, row.status, [
            row.order_type || "--", row.entry_price ? `入场 ${num(row.entry_price)}` : "",
            row.quantity ? `数量 ${num(row.quantity)}` : "", row.order_status ? `订单 ${row.order_status}` : "",
            row.updated_at || "",
        ], row.summary, [...(row.missing_fields || []), ...(row.reasons || []), row.quote_risk_usdt ? `风险 ${num(row.quote_risk_usdt)}U` : ""]);
    }

    function renderUpdate(row) {
        return item(`${actionLabel(row.action)} ${row.symbol ? `· ${esc(row.symbol)}` : ""}`, row.status, [
            row.close_fraction ? `比例 ${Math.round(row.close_fraction * 100)}%` : "",
            sideLabel(row.side), row.updated_at || "",
        ], row.summary, row.reasons || []);
    }

    function renderOrder(row) {
        return item(`${esc(row.symbol)} ${sideLabel(row.side)}`, row.status || row.intent_status, [
            row.order_type || "--", row.entry_price ? `价格 ${num(row.entry_price)}` : "",
            row.quantity ? `数量 ${num(row.quantity)}` : "", row.updated_at || "",
        ], row.error_message || exchangeText(row), [row.exchange_order_id ? `订单号 ${row.exchange_order_id}` : ""]);
    }
    ...
    function item(title, status, meta, text, tags) {
        return `<article class="signal-item"><div class="signal-top"><div class="signal-title">${title}</div>${pill(status)}</div>
            ...
            <div class="signal-tags">${tags.filter(Boolean).slice(0, 4).map((tag) => `<span class="signal-tag">${esc(tag)}</span>`).join("")}</div></article>`;
    }
    ...
    function pill(status) {
        return `<span class="signal-pill ${pillClass(status)}">${esc(statusLabel(status))}</span>`;
    }

    function upsert(bucket, row, key) {
        state.signals[bucket] = [row, ...state.signals[bucket].filter((item) => item[key] !== row[key])].slice(0, 6);
    }
    function remove(bucket, value) { state.signals[bucket] = state.signals[bucket].filter((item) => item.source_log_id !== value); }
    function lastOrderText(order) { return order ? `${order.symbol || "--"} / ${statusLabel(order.status)}` : "暂无记录"; }
    function exchangeText(row) { return row.exchange_order_id ? `交易所订单 ${row.exchange_order_id}` : "等待订单回报"; }
    function pillClass(status) { return ["ready", "submitted", "dry_run"].includes(status) ? "pill-good" : ["blocked", "needs_review", "audit_only"].includes(status) ? "pill-warn" : status === "failed" ? "pill-bad" : ""; }
    function statusLabel(status) { return ({ ready: "可执行", submitted: "已提交", dry_run: "模拟", blocked: "已阻止", failed: "失败", needs_review: "待确认", audit_only: "只记录", parsed: "已解析", missing: "缺少凭证" })[status] || status || "--"; }
    function actionLabel(action) { return ({ close: "平仓", take_partial_profit: "部分止盈", move_stop_to_breakeven: "保本", hold: "继续持有", add_layer: "补仓", risk_modifier: "调整风险", take_profit_hit: "止盈触发" })[action] || action || "--"; }
    function sideLabel(side) { return ({ long: "多", short: "空" })[side] || "--"; }
    function num(value) { return Number(value || 0).toLocaleString("zh-CN", { maximumFractionDigits: 4 }); }
    function esc(value) { return KMT.escapeHtml(value == null ? "" : String(value)); }
